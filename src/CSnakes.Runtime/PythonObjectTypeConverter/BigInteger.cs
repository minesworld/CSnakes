...
internal partial class PythonObjectTypeConverter
{
    ...
    {
        using PythonObject pyUnicode = PythonObject.Create(CPython.CAPI.Proxy.AsPyUnicodeObject(integer.ToString()));
        return PythonObject.Create(CPython.CAPI.Proxy.PyLong_FromUnicodeObject(pyUnicode.DangerousGetHandle(), 10));
    }
}
